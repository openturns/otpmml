...
result_sample.setDescription(
    ot.Description(["E", "F", "L", "I", "expected", "nnet", "difference"]))
print(result_sample)
...
for x in input_sample:
    print("x=%s, model gradient=%s, neural gradient=%s" %
          (x, poutre.gradient(x), neuralNetwork.gradient(x)))
    print("x=%s, model hessian=%s, neural hessian=%s" %
          (x, poutre.hessian(x), neuralNetwork.hessian(x)))
